...
### Spool timestamp reference and time domains

Summary spool data uses two distinct time domains:

1. **SessionModeEntries (field 6) & ClockB (field 40)** are in the **AS11 internal
   clock domain**. The AS11's internal RTC drifts relative to NTP, and these
   timestamps reflect that drifting clock. Converting them to true NTP-synced
   UTC epoch milliseconds requires adding `clock_drift_ms` (`drift = NTP - AS11`).
   In `build_str_mask_events()`, `entry_ts + clock_drift_ms` correctly aligns
   session intervals with NTP-based EDF headers, DATALOG filenames, and event
   annotations.

2. **PeriodStart (field 2) & PeriodEnd (field 3)** are **nominal calendar noon
   boundaries**, not sensor timestamps. They represent local noon in the AS11's
   configured timezone and span exactly 24 hours (1440 minutes / 86,400,000 ms).
   Because they are fixed calendar anchors rather than RTC event readings, they
   do not carry clock drift and must not be shifted by `clock_drift_ms`.

> **Correction Note (2026-09-18):**
> An earlier version of this section concluded that all summary spool timestamps
> were "in NTP-synced UTC epoch milliseconds... verified by matching ClockB exactly
> to events.snt TherapyStop timestamps."
>
> That deduction was invalid. Live `events.snt` notifications record `reportTime`
> emitted by the AS11 directly over BLE. This `reportTime` is generated by the
> AS11's internal RTC and is therefore in the **AS11 clock domain** (the firmware
> applies `+ clock_drift_ms` when converting `events.snt` entries to NTP).
>
> Matching `ClockB` against `events.snt TherapyStop` therefore proved that `ClockB`
> is in the **AS11 clock domain**, directly disproving the premise that the spool
> is NTP-synced. Field 6 session entries drift along with the AS11 clock and require
> `+ clock_drift_ms`, while nominal reporting bounds (`PeriodStart`/`PeriodEnd`)
> are drift-invariant calendar boundaries.
